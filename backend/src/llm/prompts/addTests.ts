...
- "transcript"   — verifiable from the call transcript alone. Default for criteria about what the bot said or did on the call.
- "received_sms" — pass iff the tester received any SMS from the bot during the call window. Use for criteria like "sends a confirmation SMS" or "texts the caller a receipt".
- "sms_content" — verifiable from the body of the SMS the bot sent (auto-fails if no SMS arrived). Use for criteria like "SMS contains the booking time" or "SMS includes a tracking link".
